...
<body>
	...
	%>
			...
			onmouseout="this.style.backgroundColor='#E8E8E8'">
				...
				<td id="title" style="width:150px;">
					...
					%>
					<a href="../cs/show.jsp?b_id=<%= b_id %>&pageNum=<%= pageNum %>"><%=b_title%></a>
				</td>
				<td><%= u_id %></td>
	...
	%>
			<a href="admin_list.jsp?pageNum=<%= startPage-pageBlock %>">이전</a>
	<%
		}
	%>
	<%
	...
	%>
			<a href="admin_list.jsp?pageNum=<%= i %>"><%= i %></a>
	<%	
		}
	%>
	<%
	...
	%>
			<a href="admin_list.jsp?pageNum=<%= startPage+pageBlock %>">다음</a>
	<%
		}
	%>
</div>
